...
rows = []
songs.each do |song|
  rows << [song[:title], song[:artist], song[:number_of_singers], song[:year], song[:popularity], song[:duration_ms]]
end

CSV.open(File.join(File.dirname(__FILE__), './songs.csv'), "wb") do |csv|
  csv << ["title", "artist", "number_of_singers", "year", "popularity", "duration_ms"]
  rows.each do |row|
    csv << row
  end
end